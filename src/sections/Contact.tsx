...
const Contact = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!payload.name || !payload.email || !payload.message) {
      setStatus('error')
      setFeedback('Please fill every field before sending your message.')
      return
    }
    ...
    if (!CONTACT_FORM_ENDPOINT) {
      setStatus('error')
      setFeedback(
        'Contact form endpoint is missing. Set VITE_CONTACT_FORM_ENDPOINT to your backend or Formspree URL.'
      )
      return
    }
      ...
      <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-2">
        ...
        >
          ...
          <button
            ...
            className="w-full rounded-full bg-primary py-3 text-sm font-semibold text-white transition hover:bg-primary/80 disabled:cursor-not-allowed disabled:opacity-60"
          >
